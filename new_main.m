...
params = NEW_get_data(problem,opts);
...
if semisupervised
    disp('semisupervised');
    ssl.fulltag=params.fulltag;
    ssl.bool = ssl.fulltag >0;
...
elseif strcmp(type,'CV') && ~recurse
	disp('no recursion')
	g = new_Balanced_TV(A,uo,gamma,V,D,k,twom,opts);
elseif strcmp(type,'CVR')
	opts.nhat=nhat;
	opts.lam=0.0; disp('setting lam=0')
	opts.solver = type;
	[g,~] = new_recursive_splitter(A,gamma,min(nhat,4),opts);
