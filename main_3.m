...
if ~exist('benchmark_flag') || ~benchmark_flag
	problem = 'PNNL4-B0';
	type = 'louvain'; % CVR, CV, louvain, newman, clauset
...
if semisupervised
	...
	ssl.bool = ssl.bool .* (params.sstag' > 0); % nonzero entires are (not)ground truth
...
nmi = nmiMeas(g,tag)
          ...
          %disp(profile('info'));
          %p = profile('info')
          %writetable(struct2table(p.FunctionTable),'profile.csv','Delimiter',',')
